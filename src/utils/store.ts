 ...
 */
class Store {
  public set( key: string, value: any, options: Option = {} ): boolean {
    const option = Object.assign({ expireDays: 7, type: Type.localStorage }, options)
    const type = option.type
    const expireDays = option.expireDays
    return type === 'cookie' ? this.setCookie(key, value, expireDays) : this.setStorage(key, value, expireDays, type)
  }

  public get(key: string, type: Type = Type.localStorage): string {
    return type === 'cookie' ? this.getCookie(key) : this.getStorage(key, type)
  }

  public remove(key: string, type: Type = Type.localStorage): boolean {
